...
public:
	AUTOEXEC(Section* configuration):Module_base(configuration) {
		...
		if (extra && !secure && !control->cmdline->FindExist("-noautoexec",true)) {
			...
			if (g_okdos.m_okdos_ready)
			{
				std::string more_bat = g_okdos.make_bat_string();
				if (!more_bat.empty())
				{
...
void SHELL_Init() {
	...
	MSG_Add("SHELL_STARTUP_BEGIN",
		...
		"\xBA \033[32mWelcome to DOSBox %s-lfn\033[37m (Updated: \033[33mFebruary 26, 2017\033[37m)             \xBA\n"
		"\xBA                                                                    \xBA\n"
