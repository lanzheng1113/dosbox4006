...
{   "DELTREE",  1,          &DOS_Shell::CMD_DELTREE,    "SHELL_DELTREE_FOR_HELP"},
...
//TODO Maybe support confirmation for *.* like dos does.	
	...
	while (res) {
		...
		if (!(attr & (DOS_ATTR_DIRECTORY|DOS_ATTR_READ_ONLY))) 
		{
			...
			strcpy(sfull,full);
			if (uselfn) 
				sprintf(sfull,"\"%s\"",full);
			if (!DOS_UnlinkFile(sfull)) 
...
void DOS_Shell::CMD_DELTREE(char * args) {
	...
	while(ScanCMDBool(args,"D")) ;
	while(ScanCMDBool(args,"X")) ;
	while(ScanCMDBool(args,"S")) ;
	while(ScanCMDBool(args,"Q")) ;
	StripSpaces(args);
	char * rem=ScanCMDRemain(args);
	if (rem) {
		WriteOut(MSG_Get("SHELL_ILLEGAL_SWITCH"),rem);
...
void DOS_Shell::CMD_COPY(char * args) {
	...
	while(sources.size()) {
		...
		if (pathTarget[strlen(pathTarget)-1]!='\\') {
			...
			{
				//not exist. and copy source is ends with "*", then assume it is a directory, and try to make it.
				if (pathSourcePre[0] && pathSourcePre[strlen(pathSourcePre)-1] == '*')
				{
					target_is_file = false;
					DOS_MakeDir(pathTarget);
					strcat(pathTarget,"\\");
				}
			}
...
static void replace(std::string& str, const std::string & before, const std::string & after, bool caseSensitive) 
{
	...
	while ((pos = tmp.find(before2, pos)) != std::string::npos) {
		...
		str.replace(pos, before2.length(), after);
...
std::string trim_left(const std::string& str) 
{
	...
	{
		std::string newstr = str.substr(pos1,str.length() - pos1);
		return newstr;
	}
}
...
void DOS_Shell::CMD_FOR(char * args){
	...
	//v1.0: the "set" is separate by comma or space, Wildcard is not supported("*","?" cannot be used).
	//
	//First, find the variable tag "%%"
	char variable_name[64] = {0};
	char* pVariableName = variable_name;
	while (*args)
	{
		if(*args == '%'){ 
			args ++;
		...
		if (isspace(*reinterpret_cast<unsigned char*>(args)))
			break;
		*pVariableName++ = *args++;
	}
	if (!variable_name[0])
	{
		WriteOut("Error,No vairable name\n");
		return;
	}
	WriteOut("var: %s\n", variable_name); //debug.

	// Then find the command to execute in the loop;
	std::string todo_cmd;
	std::string str_args(args), upstr(args);
	upcase(upstr);
	std::string::size_type n = upstr.find("DO");
	if (n == std::string::npos)
	{
		WriteOut("Error,No do command.\n");
		return;
	}
	todo_cmd = str_args.substr(n+2);
	todo_cmd = trim_left(todo_cmd);
	WriteOut("Do command: %s\n", todo_cmd.c_str()); //debug.

	// Then find the "set" value;
	std::vector<std::string> condition_set;
	std::string str(args), up_str(args);
	upcase(up_str);
	std::string::size_type in_tag = up_str.find("IN"); //< find "in"
	if (in_tag == std::string::npos)
	{
		WriteOut("Error, no IN.\n");
		return;
	}
	std::string::size_type left_b = str.find('(', in_tag);
	if (left_b == std::string::npos)
	{
		WriteOut("Error, no \'(\'.\n");
		return;
	}
	std::string::size_type right_b = str.find(')', left_b);
	if (right_b == std::string::npos)
	{
		WriteOut("Error, no \')\'.\n");
		return;
	}
	// Now we get a sub-string between left and right brace.
	std::string set_val_str = str.substr(left_b+1, right_b-left_b-1);
	WriteOut("%s\n", set_val_str.c_str());
	//
	std::vector<std::string> set_values = split(set_val_str, "\t ,"); 
	for (int i=0; i!=set_values.size(); i++)
	{
		WriteOut("%s | ", set_values[i].c_str()); //debug.
	}
	for (int i=0; i!=set_values.size(); i++)
	{
		SetEnv(variable_name,set_values[i].c_str()); //Maybe no need to do it, see the replacement bellow.
		//replace variables in command
		std::string replace_from = std::string("%%") + variable_name;
		std::string todo_cmd_in_loop(todo_cmd);
		replace(todo_cmd_in_loop, replace_from, set_values[i], false);

		replace_from = std::string("%") + variable_name;
		replace(todo_cmd_in_loop, replace_from, set_values[i], false);

		WriteOut("Do in for loop [%s]\n", todo_cmd_in_loop.c_str()); //debug.
		char* temp = new char [todo_cmd_in_loop.length()+1];
		memset(temp, 0, todo_cmd_in_loop.length()+1);
		strcpy(temp, todo_cmd_in_loop.c_str());
		DoCommand(temp);
		delete [] temp;
	}
}
