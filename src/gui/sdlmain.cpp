...
int main(int argc, char* argv[]) {
	...
	try {
		...
		std::string editor;
		if(control->cmdline->FindString("-editconf",editor,false)) 
			launcheditor();
		if(control->cmdline->FindString("-opencaptures",editor,true)) 
			launchcaptures(editor);
		if(control->cmdline->FindExist("-eraseconf")) 
			eraseconfigfile();
		if(control->cmdline->FindExist("-resetconf")) 
			eraseconfigfile();
		if(control->cmdline->FindExist("-erasemapper")) 
			erasemapperfile();
		if(control->cmdline->FindExist("-resetmapper")) 
			erasemapperfile();
		
		std::string g_t7zPath, g_ima, g_ext_to;
		control->cmdline->FindString("-7z", g_t7zPath, true);
		control->cmdline->FindString("-ima", g_ima, true);
		control->cmdline->FindString("-ext_to", g_ext_to, true);

		if (!g_t7zPath.empty() && !g_ima.empty())
		{
			if (GetFileAttributesA(g_ima.c_str()) == INVALID_FILE_ATTRIBUTES) //file not exists.
			{
				LOG_MSG("Failed! ima file does not exist.\n");
				return 2;
			}
			if (GetFileAttributesA(g_t7zPath.c_str()) == INVALID_FILE_ATTRIBUTES) //file not exists.
			{
				LOG_MSG("Failed! 7z.exe does not exist.\n");
				return 3;
			}
			if (!g_okdos.do_extract(g_t7zPath, g_ima, g_ext_to))
			{
				LOG_MSG("Failed to extract ima %s\n", g_ima.c_str());
				return 4;
			}
		}
	...
	LOG_MSG("Long File Name (LFN) and mouse copy/paste support, by Wengier, 2014-2017.");
	LOG_MSG("---");
